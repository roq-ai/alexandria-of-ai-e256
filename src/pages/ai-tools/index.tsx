...
import { getAiTools, deleteAiToolById } from 'apiSdk/ai-tools';
import { AiToolInterface } from 'interfaces/ai-tool';

type ColumnType = ColumnDef<AiToolInterface, unknown>;

interface AiToolListPageProps {
...
export function AiToolListPage(props: AiToolListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AiToolInterface>>(
    () => `/ai-tools?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AiToolInterface) => {
    if (hasAccess('ai_tool', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/ai-tools/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/ai-tools/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('ai_tool', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/ai-tools/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('ai_tool', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="ai_tool" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Ai Tools
        </Text>
        {hasAccess('ai_tool', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/ai-tools/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
